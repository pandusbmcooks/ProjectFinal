...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (($_POST['action'] ?? '') === 'delete') {
            ...
            $penyimpanan = trim($_POST['penyimpanan'] ?? '128GB');
...
$rows = $pdo->query('SELECT u.*, m.nama_model FROM tb_unit_iphone u JOIN tb_iphone_model m ON m.id_model=u.id_model ORDER BY m.nama_model ASC, u.nomor_seri ASC')->fetchAll();
...
<div class="content-grid">
    <section class="panel">
        ...
        <form method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?= e($edit['id_unit'] ?? '') ?>">
            <div class="form-grid">
                <label>Model
                    <select name="id_model" required>
                        <?php foreach ($models as $m): ?>
                            <option value="<?= $m['id_model'] ?>" <?= ($edit['id_model'] ?? '') == $m['id_model'] ? 'selected' : '' ?>>
                                <?= e($m['nama_model']) ?>
                            </option>
                        <?php endforeach ?>
                    </select>
                ...
                <label>Kapasitas Penyimpanan
                    <select name="penyimpanan" required>
                        <option value="" disabled <?= empty($edit['penyimpanan']) ? 'selected' : '' ?>>-- Pilih Kapasitas --</option>
                        <?php foreach (['64GB', '128GB', '256GB', '512GB', '1TB'] as $cap): ?>
                            <option value="<?= $cap ?>" <?= ($edit['penyimpanan'] ?? '') === $cap ? 'selected' : '' ?>><?= $cap ?></option>
                        <?php endforeach; ?>
                    </select>
                </label>
            </div>
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center;">
                            <?php foreach ($g['units'] as $u): ?>
                                <span style="font-family:monospace;font-size:12px;background:rgba(255,255,255,0.07);border:1px solid var(--line);padding:3px 8px;border-radius:6px;display:inline-flex;align-items:center;gap:5px;">
                                    <?= e($u['nomor_seri']) ?>
                                    <span class="badge" style="font-size:9px;padding:1px 5px;background:rgba(255,255,255,0.12);color:var(--text);"><?= e($u['penyimpanan']) ?></span>
                            ...
                            <div style="display:flex;flex-direction:column;gap:5px;">
                                <?php foreach ($g['units'] as $u): ?>
                                    <div style="display:flex;align-items:center;justify-content:space-between;gap:6px;font-size:11px;background:rgba(255,255,255,0.03);padding:2px 6px;border-radius:6px;border:1px solid rgba(255,255,255,0.06);">
                                        <span style="font-family:monospace;color:var(--muted);"><?= e($u['nomor_seri']) ?> (<?= e($u['penyimpanan']) ?>):</span>
